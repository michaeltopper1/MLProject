...
We can see from Figure \ref{relationship} that the predictors maximum playtime, users rated, decade published, and number of expansions all have a positive correlation with average ratings. Due to this relationship, we expected these to be major predictors in our model. In addition, Figure \ref{popular} shows the distribution of board game types. From this, we hypothesized that board game type plays a role in score of a game. For instance, card games may score systematically higher than adventure games. Furthermore, we can see from Figure \ref{designer} that in the 1990s-2010s, a few designers were extremely prominent in the game designing business. Because of this, we suspected that a top designer has a role in the prediction of whether a game received a high average score. 
...
```{r, fig.cap = "\\label{relationship}Relationship of Features", fig.width=6, fig.height=3}
...
```{r treegraph, fig.cap="\\label{DecisionTree}Decision Tree", fig.width=6, fig.height=4}
library(tree)
...
```{r prunetree, fig.cap="\\label{Prunetree}Pruned Decision Tree", fig.width=6, fig.height=4}
tree.prune = prune.misclass(tree.fit, best = optimal.size)
plot(tree.prune)
text(tree.prune, pretty = 0, cex = 0.8)
title("Pruned Tree Using Cross Validation for Optimal Size")
```
```{r}
...
plot(rf.tree)
title(main = "Error Rate for the Random Forest", xlab = "Number of Trees", ylab = "Error")
...
```{r importanceplot, fig.cap="\\label{importanceplot}Importance Plot", fig.width=5, fig.height=5}
varImpPlot(rf.tree)
```
...
# for (i in 2:9) {
#   rf.tree.loop = randomForest(above7 ~. -designer, data = train.dat, mtry = i, ntree = 500, importance = T)
...
To begin, we run a logistic regression using the variables described above. The training rate versus cutoff is displayed below
