...
danny.dat = danny.dat %>% mutate(top.ten = as.factor(ifelse(designer %in% top.ten.names, 1, 0))) %>% drop_na()
danny.dat = danny.dat %>% mutate_if(is.character, as.factor)
...
rf.tree = randomForest(above7 ~. -designer, data = train.dat.m, mtry = 3, ntree = 500, importance = T)
...
```{r}
yhat.rf = predict(rf.tree, newdata = test.dat.m)
rf.err = table(pred = yhat.rf, truth = test.dat.m$above7)
rf.err
